...
void print(const char* label, const Ordered_list<T, OF>& in_list)
{
    ...
    for(typename Ordered_list<T, OF>::const_Iterator it = in_list.begin(); it != in_list.end(); it++) {
        cout << ' ' << *it;
    }
    cout << endl;
}
...
/* Sample output
 -bash-4.2$ g++ -std=c++14 -pedantic-errors -Wall Ordered_list_String_exception_safety_demo.cpp String.cpp Utility.cpp p2_globals.cpp
 -bash-4.2$ ./a.out
 Ctor: "A"
